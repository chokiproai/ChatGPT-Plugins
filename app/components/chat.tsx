...
function _Chat() {
  ...
  // Shortcut key Shortcut Keys
  const [showShortcutKeyModal, setShowShortcutKeyModal] = useState(false);

  useEffect(() => {
    const handleKeyDown = (event: any) => {
      // Open the new chat Command + Shift + O
      if (
      ...
      // Copy the last code block Command + Shift + ;
      else if (
        (event.metaKey || event.ctrlKey) &&
        event.shiftKey &&
      ...
      // Copy the last reply COMMAND + Shift + C
      else if (
        (event.metaKey || event.ctrlKey) &&
        event.shiftKey &&
